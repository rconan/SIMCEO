   ...
   (isempty(item) && any(size(item))) ||jsonopt('ArrayToStruct',0,varargin{:}))
    ...
    if(isempty(name))
        ...
        if(numel(item)==1 && jsonopt('SingletArray',0,varargin{:})==0)
            numtxt=regexprep(regexprep(matdata2ubjson(item,level+1,varargin{:}),'^\[',''),']','','once');
...
if(isa(num,'single'))
  val=['d' data2byte(swapbytes(single(num)),'uint8')];
else
  val=['D' data2byte(swapbytes(double(num)),'uint8')];
end
%%-------------------------------------------------------------------------
